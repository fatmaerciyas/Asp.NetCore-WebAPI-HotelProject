    ...
    public class StaffController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            if(responseMessage.IsSuccessStatusCode) //adresten basarili durum kodu donerse
            {
                ...
                var values = JsonConvert.DeserializeObject<IEnumerable<StaffViewModel>>(jsonData); //Json turundeki veriyi de deserialize ettik
                return View(values);
            }
            return View();
        }
        ...
        [HttpPost]
        public async Task<IActionResult> Add(StaffViewModel model)
        {
        ...
        {
            var client = httpClientFactory.CreateClient();
            var responseMessage = await client.DeleteAsync($"http://localhost:5181/api/Staffs/{id}"); //id ye gore veri sil
            if (responseMessage.IsSuccessStatusCode) //adresten basarili durum kodu donerse
            {
                return RedirectToAction("Index");
        ...
        {
            var client = httpClientFactory.CreateClient();
            var responseMessage = await client.GetAsync($"http://localhost:5181/api/Staffs/{id}"); //id ye gore veri getir
            if (responseMessage.IsSuccessStatusCode) //adresten basarili durum kodu donerse
            {
                var jsonData = await responseMessage.Content.ReadAsStringAsync();
                var values = JsonConvert.DeserializeObject<StaffViewModel>(jsonData); //sadece 1 veri gelecek Gelen json veriyi StaffViewModel'e cevir
                return View(values);
        ...
        [HttpPost]
        public async Task<IActionResult> Update(StaffViewModel model)
        {
            var client = httpClientFactory.CreateClient();
            var jsonData = JsonConvert.SerializeObject(model); // gelen veriyi jsona cevirdik
            StringContent stringContent = new StringContent(jsonData, Encoding.UTF8, "application/json");// gelen veriyi turkce karakter destekleyen jsona cevirdik
