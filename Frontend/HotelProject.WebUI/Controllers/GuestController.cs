...
namespace HotelProject.WebUI.Controllers
{
    public class GuestController : Controller
    {
        ...
        string baseUrl = "http://localhost:5181/api/Guests";
        ...
        public async Task<IActionResult> Index()
        {
            ...
            if (responseMessage.IsSuccessStatusCode) //adresten basarili durum kodu donerse
            {
                ...
                var values = JsonConvert.DeserializeObject<IEnumerable<ResultGuestDto>>(jsonData); //Json turundeki veriyi de deserialize ettik
                return View(values);
            }
            return View();
        }
        ...
        [HttpPost]
        public async Task<IActionResult> Add(CreateGuestDto model)
        {
        ...
        {
            var client = httpClientFactory.CreateClient();
            var responseMessage = await client.DeleteAsync($"http://localhost:5181/api/Guests/{id}"); //id ye gore veri sil
            if (responseMessage.IsSuccessStatusCode) //adresten basarili durum kodu donerse
            {
                return RedirectToAction("Index");
        ...
        {
            var client = httpClientFactory.CreateClient();
            var responseMessage = await client.GetAsync($"http://localhost:5181/api/Guests/{id}"); //id ye gore veri getir
            if (responseMessage.IsSuccessStatusCode) //adresten basarili durum kodu donerse
            {
                var jsonData = await responseMessage.Content.ReadAsStringAsync();
                var values = JsonConvert.DeserializeObject<UpdateGuestDto>(jsonData); //sadece 1 veri gelecek Gelen json veriyi StaffViewModel'e cevir
                return View(values);
        ...
        [HttpPost]
        public async Task<IActionResult> Update(UpdateGuestDto model)
        {
            if (ModelState.IsValid)
            {
                var client = httpClientFactory.CreateClient();
