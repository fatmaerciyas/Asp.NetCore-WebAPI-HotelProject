...
namespace HotelProject.WebUI.Areas.Admin.Controllers
{
    ...
    public class RoomAdminController : Controller
    {
        ...
        [HttpPost]
        public async Task<IActionResult> Add(CreateRoomDto model)
        {
        ...
        public async Task<IActionResult> Update(int id)
        {
            ...
            if (responseMessage.IsSuccessStatusCode) //adresten basarili durum kodu donerse
            {
                ...
                var values = JsonConvert.DeserializeObject<RoomUpdateDto>(jsonData); //sadece 1 veri gelecek Gelen json veriyi StaffViewModel'e cevir
                return View(values);
        ...
        [HttpPost]
        public async Task<IActionResult> Update(RoomUpdateDto model)
        {
            var client = httpClientFactory.CreateClient();
            var jsonData = JsonConvert.SerializeObject(model); // gelen veriyi jsona cevirdik
            StringContent stringContent = new StringContent(jsonData, Encoding.UTF8, "application/json");// gelen veriyi turkce karakter destekleyen jsona cevirdik
